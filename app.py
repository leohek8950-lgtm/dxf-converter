...
def parse_drawing_with_gemini(img_bytes: bytes):
    ...
    prompt = """
    Ты — инженер-конструктор и эксперт по расшифровке рукописных эскизов и чертежей ЧПУ. 
    Твоя задача — распарсить изображенный эскиз (включая наброски от руки на клетчатой или чистой бумаге) и вернуть JSON для построения 3D-модели.

    ПРАВИЛА ОБРАБОТКИ РУКОПИСНЫХ ЭСКИЗОВ И НАБРОСКОВ:
    1. ОРИЕНТАЦИЯ И ПОВОРОТ: Изображение может быть повернуто на 90, 180 или 270 градусов. Мысленно поверни эскиз так, чтобы деталь располагалась горизонтально или вертикально вдоль оси симметрии/вращения.
    2. РУКОПИСНЫЙ ТЕКСТ И РАЗМЕРЫ: Тщательно распознавай рукописные цифры (например: 20, 25, 30 и т.д.), выносные линии и стрелки, даже если они нарисованы синей ручкой или карандашом.
    3. ИГНОРИРОВАНИЕ ШУМА: Игнорируй сетку тетрадных клеток, фоновые тени, пятна и случайные штрихи. Focus только на контуре детали и размерных числах.
    4. ТОЧНОСТЬ ГЕОМЕТРИИ: 
       - Распознай ступени цилиндров слева направо (или сверху вниз вдоль главной оси).
       - Отрезки со стрелками, указывающие на ширину тела, считай диаметрами (`start_diameter`, `end_diameter`).
       - Отрезки со стрелками вдоль длины считай длинной секции (`length`).
    5. ЕСЛИ ЭТО ВРАЩАТЕЛЬНАЯ ДЕТАЛЬ: Сформируй массив `outer_profile` из цилиндрических секций (`cylinder`).

    Пример детали с фото (2 ступени: диаметр 20 длиной 25, затем диаметр 30 длиной 25):
    {
      "part_name": "Рукописный эскиз",
      "total_length": 50.0,
      "outer_profile": [
        {"type": "cylinder", "start_diameter": 20.0, "end_diameter": 20.0, "length": 25.0},
        {"type": "cylinder", "start_diameter": 30.0, "end_diameter": 30.0, "length": 25.0}
      ],
      "bores": []
    }

    Верни ТОЛЬКО чистый JSON без markdown, тегов и комментариев.
    """
